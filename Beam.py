...
def beam_search(src, model, src_vocab, trg_vocab, opt):
    ...
    if ind is None:
        ...
        outputs = outputs.detach().cpu().numpy()
        try:
            return ' '.join([trg_vocab.itos[tok] for tok in outputs[ind][1:length] if
                             tok != pad_token and tok != eos_token and tok != bos_token])
        except:
            return ' '.join([trg_vocab.itos[tok] for tok in outputs[ind][1:] if
                             tok != pad_token and tok != eos_token and tok != bos_token])
